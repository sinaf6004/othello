    ...
    while (1)
    {
        ...
        if (t == 1)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    char holdMember[10];
                    strcpy(holdMember, member1.name);
                    strcpy(member1.name, member2.name);
                    strcpy(member2.name, holdMember);
                }

                int lastFileNUmber = prointPreviousGames(member1.name, member2.name);
        ...
        if (t == 2)
        {
            ...
            for (int i = 0; i < (60 + extraCounter); i++)
            {
                ...
                if (quit == 1)
                {
                    ...
                    fclose(filePointer);
                    ////////////////////////////////////////////////////////////////////////////////////////////
                    fp = fopen("pointfilenames.txt", "r");
                    fread(&table, sizeof(table), 1, fp);
                    fclose(fp);
                    strcpy(table.array[table.lastFilled], globalPointName1);
                    table.lastFilled++;
                    fp = fopen("pointfilenames.txt", "w");
        ...
        if (t == 5)
        {
            ...
            int t = 0;
            int a = 0;
            for (int i = 0; i < table.lastFilled; i++)
            {
                for (int j = 0; j < i; j++)
                {
                    if (strcmp(table.array[i], table.array[j]) == 0)
                    {
                        t = 1;
                        break;
                    }
                ...
                if (t == 0)
                {
                    fp = fopen(table.array[i], "r");
                    fread(&glob, sizeof(glob), 1, fp);
                    fclose(fp);
                    last[a].name[0] = '\0';
                    strcpy(last[a].name, glob.name);
                    last[a].point = glob.point;
                    a++;
                }
            }
            for (int i = 0; i < a; i++)
            {
                for (int i = 0; i < a - 1; i++)
                {
                    if (last[i].point < last[i + 1].point)
                    {
                        globalPoint hold;
                        hold = last[i];
                        last[i] = last[i + 1];
